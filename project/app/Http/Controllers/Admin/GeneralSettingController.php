...
class GeneralSettingController extends AdminBaseController
{
    ...
    public function updateThemeColors(Request $request)
    {
        ...
        $data->theme1_color = $this->normalizeThemeColor($request->theme1_color, $data->theme1_color ?: $data->colors);
        $data->theme2_color = $this->normalizeThemeColor($request->theme2_color, $data->theme2_color ?: $data->colors);
        $data->theme3_color = $this->normalizeThemeColor($request->theme3_color, $data->theme3_color ?: $data->colors);
    ...
    private function normalizeThemeColor(?string $value, string $fallback): string
    {
        if (!$value) {
            return $fallback;
        }
