  ...
  function loadMedia360Manifest(mode, callback) {
    ...
    $(document).ready(function() {
          ...
          $('#media-advanced-collapse').on('shown.bs.collapse', function() {
            ...
            mlog('advanced media accordion opened');
          ensure360Script().then(function() {
              mlog('360 script ready, loading manifest…');
              loadMedia360Manifest('append');
                ...
                mlog('ensure360Script failed', e);
            });
            ensureModelViewerScripts()
                .then(function() { mlog('model-viewer scripts ready'); })
                .catch(function(e) { mlog('ensureModelViewerScripts failed', e); });
        });
...
(function($) {
  ...
  $(document).ready(function() {
    mlog('page ready (hotspot/3d dom snapshot)', {
      hotspot_base: $('#media_hotspot_base').val(),
      dots: $('.media-hotspot-dot').length,
      items: $('.media-hotspot-item').length,
      model_src: $('#media_3d_viewer').attr('src') || null,
      model_enabled: $('#media_3d_enabled').is(':checked')
    });
  ...
  <script type="text/javascript">
    ...
    function renderFrameHotspots(frameIndex) {
      ...
      $('.media-hotspot-item').each(function() {
        ...
        // Helpful one-time snapshot for prefetched hotspots (dev mode)
        $(function() {
            setTimeout(function() {
                mlog('post-load snapshot (hotspots)', {
                    base: $('#media_hotspot_base').val(),
                    dots: $('.media-hotspot-dot').length,
                    items: $('.media-hotspot-item').length
                });
