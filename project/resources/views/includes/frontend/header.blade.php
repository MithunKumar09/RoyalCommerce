<header class="header-section position-relative z-2 header-stikcy">
    ...
    @php
        // Theme selection (theme4 is the homepage default; query param can preview others)
        $previewTheme = request()->query('theme');
        $activeTheme = in_array($previewTheme, ['theme1', 'theme2', 'theme3', 'theme4'], true) ? $previewTheme : 'theme4';
    @endphp

    @if ($activeTheme === 'theme4')
        {{-- Theme 4: replace ONLY the lower header row with reference design --}}
        @include('includes.frontend.theme4_header_top')
    @else
